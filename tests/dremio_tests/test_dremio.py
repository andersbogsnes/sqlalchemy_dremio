...
@pytest.mark.parametrize(
    "param,column_name,expected",
    [(1, "int_col", 1), ("ZZZZZZZZZZZZ", "varchar_col", 1)],
)
def test_can_use_where_statement(
    test_table: sa.Table, db: Connection, column_name: str, param: Any, expected: Any
):
    sql = sa.select(test_table.c.int_col).where(
        getattr(test_table.c, column_name) == param
    )
...
def test_cte_works_correctly(test_table: sa.Table, db: Connection, subquery_type: str):
    sql = sa.select(test_table.c.int_col.label("my_id")).where(
        test_table.c.int_col == 1
    )
...
@pytest.mark.xfail(
    reason="Need to implement Dremio type hierarchy to handle Binary encoding"
)
...
def test_has_table(
    engine: sa.Engine, db: Connection, table_name: str, schema: str, exists: bool
):
